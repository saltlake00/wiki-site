...
> **판단**: 프레임을 하나씩 만드는 대신 **짧은 비디오를 만들어 프레임을 뽑는** 경로다.
> 애니메이션 프레임 간 일관성이 낫지만 **로컬 GPU(8GB+ VRAM)와 ComfyUI 셋업이 전제**라
> 진입 비용이 가장 크다. FPS는 8~12에서 고르고 **24는 비추천**(스프라이트에 과잉).
> **다음**: 워크플로우 A의 Step 2로 프레임이 충분하면 이 경로는 열지 않는다.
> 열 때는 캐릭터 1종으로 먼저 시험한다.
